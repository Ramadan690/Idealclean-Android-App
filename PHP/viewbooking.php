<?php 
	...
	$result = array();
	$result['data'] = array();
	$sql= 'SELECT T.id,B.user,B.address,B.date,B.total,status,payment,CleanerName,CleanerTel from transactions AS T JOIN booking AS B ON B.transaction_id = T.ID';
	$responce = mysqli_query($connection,$sql) or die( mysqli_error($connection));;
	echo "----------------------<h5>Bookings Approval</h5>-----------------------<Br>";
